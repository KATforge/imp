   ...
   case "$AI_PROVIDER" in
      claude)
         ...
         result=$(printf '%s' "$prompt" \
            | timeout 120 claude -p --model "$model" --max-turns 1 --tools "" 2> /dev/null)
         ;;
      ollama)
         ...
         result=$(printf '%s' "$prompt" \
            | jq -Rs --arg m "$model" '{model:$m, prompt:., stream:false}' \
            | curl -sf --max-time 30 localhost:11434/api/generate -d @- \
            | jq -r '.response') || {
