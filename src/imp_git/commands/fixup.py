...
def fixup (
   ...
   if not git.staged_files ():
      console.hint ("imp add <files> first")
   ...
   if target:
      ...
      shown = git.show_oneline (sha)
      subject = shown.split (" ", 1) [1] if " " in shown else ""
      console.item (f"{sha [:12]}  {subject}")
...
def _do_fixup (sha: str, squash: bool, auto: bool):
   ...
   if not squash:
      console.hint (f"imp fixup --squash, or imp rebase -i --autosquash {sha}^")
      return
