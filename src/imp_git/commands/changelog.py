...
from imp_git import ai, console, git, prompts, version
...
def _tidy (yes: bool, changelog_path: Path):
   ...
   old = [ line for line in before.splitlines () if line.strip ().startswith ("- ") ]
   new = [ line for line in after.splitlines () if line.strip ().startswith ("- ") ]
