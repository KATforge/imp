...
def _recovery_suggestions (sha: str, meta: dict [str, str]):
   ...
   console.item (f"inspect:       imp show {sha [:12]}")
   console.item (f"branch from:   imp branch rescued-{sha [:7]} {sha}")
   console.item (f"cherry-pick:   imp cherry-pick {sha}")
   console.item (f"hard reset:    imp reset --hard {sha}   (DESTRUCTIVE)")
   console.out.print ()
