...
def resolve (
   ...
   for i, path in enumerate (files):
      ...
      if MARKER in result and yes:
         console.err ("AI could not remove every conflict marker")
         num_skipped += 1
         continue
      ...
      if choice == "Accept":
         if MARKER in result:
            console.err ("Conflict markers remain; edit the result or choose a side")
            num_skipped += 1
            continue
         if validate.adds_provenance (content, result):
            console.err ("AI attribution or an actor ID was added; edit the result or choose a side")
            num_skipped += 1
            continue
         ...
         if MARKER in edited:
            console.err ("Conflict markers remain; file was not staged")
            num_skipped += 1
         elif validate.adds_provenance (content, edited):
            console.err ("AI attribution or an actor ID was added; file was not staged")
            num_skipped += 1
         elif edited.strip ():
   ...
   if num_skipped == 0 and num_resolved > 0:
      if git.merge_in_progress ():
         console.hint ("imp merge --continue")
      elif git.rebase_in_progress ():
         console.hint ("imp rebase --continue")
      else:
