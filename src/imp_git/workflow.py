...
from imp_git import console, git, validate


def _commit (msg: str, on_cancel: Callable [[], None] | None, **kwargs):
   if not validate.publishable (msg):
      if on_cancel:
         on_cancel ()
      console.fatal ("Commit message must not include AI attribution or actor IDs")

   git.commit (msg, **kwargs)
...
def integrate (
   ...
   *,
   ...
   from imp_git.commands.resolve import resolve as resolve_cmd
...
def review_commit (
   ...
   **commit_kwargs,
   ...
   if yes:
      console.item (msg)
      _commit (msg, on_cancel, **commit_kwargs)
      return msg
