...
from imp.commands.release import _push_commits, require_tag_available
...
def collapse (
   ...
   since: str = typer.Option ("", "--since", "-s", help="Keep this version; delete every tag above it. Defaults to the highest stable tag below <new_version>."),
   ...
   if not will_push:
      console.hint (f"push later: git push origin {new_tag}")
      return
